...
export default function DestinationsPageSection({
  ...
  const destinationPackages = Array.isArray(data?.packagesData)
    ? data.packagesData
    : data?.packageData
      ? [data.packageData]
      : [];
  const [packages, setPackages] = useState<any[]>(initialPackages || destinationPackages);
  const [loading, setLoading] = useState(false);
  const [bookingPackage, setBookingPackage] = useState<any>(null);
  const [detailPackage, setDetailPackage] = useState<any>(null);

  useEffect(() => setPackages(initialPackages || destinationPackages), [initialPackages, data?.packagesData]);
  ...
  return (
    <section className="pt-12 pb-16 bg-blue-lt">
      <div className="max-w-[1400px] mx-auto px-5">
        ...
        {!loading && packages.length === 0 && (
            <p className="text-center text-primary">No packages have been added to this destination yet.</p>
        )}
        ...
        {!loading && packages.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {packages.map((pkg, index) => {
              ...
              const heroImage = cardData.bannerImage || pkg.image || pkg.featuredImage || "/images_BHT/sections/hajj_1.jpg";
              const price = pkg.startingPrice
                ...
                : "Contact us";
              const destinationLabel = cardData.destination || pkg.destination || pkg.destinationTitle || "Selected destination";
              const packageTitle = String(pkg.title || pkg.packageTitle || cardData.packageTitle || destinationLabel || "Destination Package").trim();
              ...
              return (
                <article key={pkg.id || index} className="bg-white rounded-[2rem] overflow-hidden shadow-[0_4px_25px_rgb(0,0,0,0.07)] border border-gray-100 flex flex-col">
                  <div className="relative h-[230px] w-full">
                    <img src={heroImage} alt={packageTitle} className="w-full h-full object-cover" />
                    ...
                    <div className="absolute bottom-4 left-5 right-5">
                      ...
                      <h3 className="max-w-full whitespace-normal break-words text-white font-serif text-2xl leading-tight font-bold">{packageTitle}</h3>
                    </div>
                  </div>
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                          ...
                          {accommodations.map((hotel: any, hotelIndex: number) => (
                            <div key={hotelIndex} className="flex gap-3 p-2.5 rounded-2xl border border-[#eef0e4] bg-[#fcfdf9] items-start">
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="text-red font-serif font-bold text-[10px] uppercase tracking-wide truncate">{hotel.name || "Accommodation"}</div>
                                ...
                                <div className="flex gap-1.5 flex-wrap">
                                  ...
                                  {hotel.nights && <span className="bg-red-lt text-white text-[9px] px-1.5 py-0.5 rounded font-bold flex items-center gap-1">{getDurationUnit(hotel.nights) === "days" ? <LucideIcons.Sun className="w-2.5 h-2.5" /> : <LucideIcons.MoonStar className="w-2.5 h-2.5" />}{hotel.nights}</span>}
                                </div>
                              </div>
                            </div>
                          ))}
                    ...
                    <div className="border-t border-gray-100 pt-4 flex items-end justify-between mb-5">
                      <div><div className="text-[10px] font-bold text-ink-soft uppercase tracking-widest mb-1">OPERATOR</div><div className="text-sm font-bold text-ink flex items-center gap-2">{cardData.operatorName || "British Hajj Travel"}<span className="bg-red text-white text-[10px] px-1.5 py-0.5 rounded font-black">{cardData.operatorRating || "4.4/5"}</span></div></div>
                      <div className="text-right"><div className="text-[10px] font-bold text-red uppercase tracking-widest mb-1">{cardData.priceSubtext || "FROM £ / PERSON"}</div><div className="text-2xl font-black text-primary leading-none">{price}</div></div>
                    </div>

                    <div className="flex flex-col sm:flex-row gap-2.5 mt-auto">
                      <button type="button" onClick={() => { if (pkg.destinationSlug) window.location.href = `/destinations/${pkg.destinationSlug}`; else setDetailPackage({ ...pkg, detailPageData: { ...(pkg.detailPageData || {}), overview: pkg.description || pkg.detailPageData?.overview } }); }} className="flex-1 py-3.5 border-2 border-primary text-primary hover:bg-primary hover:text-white text-xs font-black rounded-xl uppercase tracking-wider transition-colors flex justify-center items-center gap-2"><LucideIcons.Eye className="w-4 h-4" /> View Detail</button>
                      <button type="button" onClick={() => setBookingPackage(pkg)} className="flex-1 py-3.5 bg-red hover:bg-white hover:border hover:border-red text-white hover:text-red text-xs font-black rounded-xl uppercase tracking-wider transition-colors flex justify-center items-center gap-2 shadow-sm"><LucideIcons.BookOpen className="w-4 h-4" /> Book Now</button>
                    </div>
                  </div>
                </article>
              );
      ...
      <PackageDetailModal isOpen={Boolean(detailPackage)} onClose={() => setDetailPackage(null)} pkg={detailPackage} />
    </section>
  );
}